...
import {fetchComponents} from "../../services/server/actions";
import {showComponentList} from "../../services/content/actions";
import PropTypes from 'prop-types';
import './style.css'

export const Schema = ({fetchComponents, showComponentList}) => {
    return <svg id="board" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 401 453" width="100%" height="600px">
        <rect id="motherboard" className="cls-1" x="0.5" y="0.5" width="400" height="452"/>
        <g id="processor" onClick={() => {
            showComponentList();
            fetchComponents('Processor', 'Процессоры');
        }}>
        ...
        <g id="videocard" onClick={() => {
            showComponentList();
            fetchComponents('VideoCard', 'Видеокарты')
        }}>
            <g>
